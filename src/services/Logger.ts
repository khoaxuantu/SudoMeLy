...
export class Logger {
    ...
    private interactionTypeReadable: {
        [key in InteractionsConstants]: string
    } = {
    ...
    logNewUser(user: User) {
        const message = `(NEW_USER) ${user.username} (${user.id}) has been added to the db`
        const chalkedMessage = `(${chalk.bold.white(
            'NEW_USER'
        )}) ${chalk.bold.green(user.username)} (${chalk.bold.blue(
            user.id
    ...
    async logStartingConsole() {
        ...
        this.console(
            chalk.blue(
                `${symbol} ${numberAlign(commandsSum)} ${chalk.bold(
                    'commands'
                )} loaded`
            ),
            'info',
        ...
        this.console(
            chalk.magenta(
                `${symbol} ${numberAlign(events.length)} ${chalk.bold(
                    'events'
                )} loaded`
            ),
            'info',
        ...
        this.console(
            chalk.red(
                `${symbol} ${numberAlign(
                    entities.length + pluginsEntitesCount
        ...
        if (apiConfig.enabled) {
            this.console(
                chalk.gray(
                    boxen(
                        ` API Server listening on port ${chalk.bold(
                            apiConfig.port
                        )} `,
                        {
                            padding: 0,
